...
class ShowRequirements extends Component{
...
componentDidMount(){
  ...
  clickHandler (id) {
    ...
    const {confirmed, ordersId, offer} = items.find((element) => {
    return element.id == id;
  });

    axios.put('http://localhost:8079/api/ordering/requirement/' + id, {confirmed, ordersId, offer})
      .then((result) => {
        browserHistory.push("/ShowRequirements/")
      });
